...
export class LobbyComponent implements OnDestroy {
  ...
  constructor(
    ...
    if (typeof localStorage !== 'undefined') {
      ...
      this.game.password = this.gameInfo
        ? JSON.parse(this.gameInfo).password
        : '';
      this.players = this.gameResponse
        ? JSON.parse(this.gameResponse).players
        : [];
      this.gameName = this.gameResponse
        ? JSON.parse(this.gameResponse).name
        : '';
      let roundId = this.gameResponse
        ? JSON.parse(this.gameResponse).currentRound
        : '';
    ...
    this.dataService.votePlayer(this.playerVote).subscribe({
      next: (response: any) => {
        ...
        if (response.status == 200) {
          ...
          console.error('Error en la respuesta, estado no es 200:', response);
        }
      },
      error: (e) => {
        switch (e.status) {
          ...
          case 403:
            this._snackBar.open(
              'The client does not have access rights to the content. Unlike 401 Unauthorized, the clients identity is known to the server.',
              'Ok',
              {
                duration: 5000,
              }
          ...
          case 429:
            this._snackBar.open(
              'The origin server requires the request to be conditional. This response is intended to prevent the "lost update" problem, where a client GETs a resources state, modifies it and PUTs it back to the server, when meanwhile a third party has modified the state on the server, leading to a conflict.',
              'Ok',
              {
                duration: 5000,
              }
